...
corrSpeed = load(fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\stats\correlation_ball','corrStatsTableSpeedPearsonDist_05_02_23'),'corrStatsTable');
...
for mov = 1:2 % rest-run-whole_trace
        ...
        for rr = 1:size(corrStatsTableSpeed,1)
            ...
            switch mov
                case 1  % rest
                    ...
                    cell_close = double(corrStatsTableSpeed{rr,12}{:}<20);
                    eData = eventStatsTable.EventRateLow(eventStatsIdx);
                    dist = corrStatsTableSpeed{rr,7}{:};
%                     movSp = [movSp;0];
                case 2  % run
                    corrMat = corrStatsTableSpeed{rr,8}{:};
                    thresh = corrStatsTableSpeed{rr,9}{:};
                    cell_close = double(corrStatsTableSpeed{rr,12}{:}<20);
...
%                     movSp = [movSp;1];
            ...
            distData = dist((cell_close == 0) & (corrMat>1e-5) & goodEIdx);
            for cc = 1:size(distData,1)
                if any(isnan(distData{cc,1}))
                    distData{cc,1} = nan*ones(1,100);
                end
            end
...
%             end
            switch mov
                case 1
                     ...
                     plot_struct(rr).dist_rest = [repmat(eData1pos,100,1),repmat(eData2pos,100,1),reshape(cell2mat(distData),[],1)];
                case 2
                     switch typ
                         case 1
                     ...
                     plot_struct(rr).dist_run = [repmat(eData1pos,100,1),repmat(eData2pos,100,1),reshape(cell2mat(distData),[],1)];
                case 3
                     switch typ
                         case 1
...
%% shuffled dist
shuffled_whole = [cell2mat(plot_table.dist_rest);cell2mat(plot_table.dist_run)];
shuffled_whole_pos = shuffled_whole(shuffled_whole(:,3)>0,:);
[fsig] = plotEvCorr_shuff(shuffled_whole_pos,0);

saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\Reviews_02_23\plots\s5\ev_rate_shuffled.fig'))
saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\Reviews_02_23\plots\s5\ev_rate_shuffled.png'))
saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\Reviews_02_23\plots\s5\ev_rate_shuffled.epsc'))
saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\Reviews_02_23\plots\s5\ev_rate_shuffled.pdf'))
...
% 
% sig_whole = [cell2mat(plot_table.sig_rest);cell2mat(plot_table.sig_run)];
% non_sig_whole = [cell2mat(plot_table.non_sig_rest);cell2mat(plot_table.non_sig_run)];
% thresh_whole = [cell2mat(plot_table.thresh_rest);cell2mat(plot_table.thresh_run)];
% 
% 
% 
% N = 20;
...
% [fsig,fnonsig] = plotEvCorr_Av_1(sig_whole,non_sig_whole,0); % Swarm plot for Non significant correlation vs event rate
