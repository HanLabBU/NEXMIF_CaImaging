...
for row =  1:size(graphStatsTable,1)
    ...
    close_run_res = mean((wcc_run(mov_run))*100);
    close_run_nonres = mean((wcc_run(~mov_run))*100);
...
subplot(1,2,1)
b1 = boxchart(1*ones(1,numel(close_WT_rest_delta)),...
    close_WT_rest_delta,...
    'BoxFaceAlpha',0,...
    'MarkerStyle','none',...
...
hold on
s1 = scatter(1,close_WT_rest_delta,48,colors(1,:),'filled');

b3 = boxchart(2*ones(1,numel(close_KO_rest_delta)),...
    close_KO_rest_delta,...
    'BoxFaceAlpha',0,...
    'MarkerStyle','none',...
...
hold on
s3 = scatter(2,close_KO_rest_delta,48,colors(3,:),'filled');
xticks([1,2])
xticklabels({'WT Rest','KO Rest'})
yy = ylim;
[hrest, prest] = ttest2(close_WT_rest_delta,close_KO_rest_delta);
text(1.5, yy(2)*1.05, sprintf('P-ttest: %.3f' ,prest),'HorizontalAlignment','center')
title({'Mean closeness centrality of','rest modulated cells - rest non modulated cells'})
axis square
...
subplot(1,2,2)
b2 = boxchart(1*ones(1,numel(close_WT_run_delta)),...
    close_WT_run_delta,...
    'BoxFaceAlpha',0,...
    'MarkerStyle','none',...
...
hold on
s2 = scatter(1,close_WT_run_delta,48,colors(2,:),'filled');

b4 = boxchart(2*ones(1,numel(close_KO_run_delta)),...
    close_KO_run_delta,...
    'BoxFaceAlpha',0,...
    'MarkerStyle','none',...
...
hold on
s4 = scatter(2,close_KO_run_delta,48,colors(4,:),'filled');
xticks([1,2])
...
title({'Mean closeness centrality of','movement modulated cells - movement non modulated'})
axis square
set(gca,'TickDir', 'out','TickLength',[0.03, 0.025], 'Color','none','LineWidth',1)
box off
ylim([-inf,7])

means = [mean(close_WT_rest_delta),mean(close_WT_run_delta),mean(close_KO_rest_delta),mean(close_KO_run_delta)]';
stds = [std(close_WT_rest_delta),std(close_WT_run_delta),std(close_KO_rest_delta),std(close_KO_run_delta)]';
...
%     figure('Units','Normalized','Position',[0 0 1 1])
    ...
    g4.NodeCData = mov_run;
    colormap jet
    axis square
    colorbar
    caxis([0 1])
    title([tt, sprintf('%s day %i Run',results(row).animal,results(row).day)])
    saveas(gcf,fullfile("U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\j_graphs\network_sigcorr\mov responsive cells",sprintf('%s day %i schematic %s.png',results(row).animal,results(row).day,tt{:})));
    saveas(gcf,fullfile("U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\j_graphs\network_sigcorr\mov responsive cells",sprintf('%s day %i schematic %s.svg',results(row).animal,results(row).day,tt{:})));
    saveas(gcf,fullfile("U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\j_graphs\network_sigcorr\mov responsive cells",sprintf('%s day %i schematic %s.fig',results(row).animal,results(row).day,tt{:})));
