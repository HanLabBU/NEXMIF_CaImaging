...
list_run_mov_val_WT = []; % All cells mov resp - corr during running 
list_run_mov_val_KO = [];
% rest
...
list_run_non_mov_val_WT = []; % All cells mov non resp - corr during running 
list_run_non_mov_val_KO = [];
% rest
...
for k =  1:numel(corrStatsTable.animal)
    ...
    if ismember(mouseName,miceWT)
        ...
        list_run_mov_val_WT = [list_run_mov_val_WT; mean(w_corr_high(w_corr_high>0 & res_cell_mat_both>0),'omitnan')];
    else
        ...
        list_run_mov_val_KO = [list_run_mov_val_KO; mean(w_corr_high(w_corr_high>0 & res_cell_mat_both>0),'omitnan')];
    end
    
        % collect all values for resting
    if ismember(mouseName,miceWT)
        list_rest_sig_mov_val_WT = [list_rest_sig_mov_val_WT; mean(w_corr_low(adj_sig_corr_low_pos & res_cell_mat_both))];
        list_rest_non_sig_mov_val_WT = [list_rest_non_sig_mov_val_WT; mean(w_corr_low(adj_sig_corr_low_non_sig & res_cell_mat_both))];
    else
        list_rest_sig_mov_val_KO = [list_rest_sig_mov_val_KO; mean(w_corr_low(adj_sig_corr_low_pos & res_cell_mat_both))];
    ...
    if ismember(mouseName,miceWT)
        ...
        list_run_non_mov_val_WT = [list_run_non_mov_val_WT; mean(w_corr_high(w_corr_high>0 & ~res_cell_mat_both),'omitnan')];
        ...
        list_run_non_mov_val_KO = [list_run_non_mov_val_KO; mean(w_corr_high(w_corr_high>0 & ~res_cell_mat_both),'omitnan')];
    
    end
    
   % collect all values for resting
    if ismember(mouseName,miceWT)
        list_rest_sig_non_mov_val_WT = [list_rest_sig_non_mov_val_WT; mean(w_corr_low(adj_sig_corr_low_pos & ~res_cell_mat_both))];
        list_rest_non_sig_non_mov_val_WT = [list_rest_non_sig_non_mov_val_WT; mean(w_corr_low(adj_sig_corr_low_non_sig & ~res_cell_mat_both))];
    else
        list_rest_sig_non_mov_val_KO = [list_rest_sig_non_mov_val_KO; mean(w_corr_low(adj_sig_corr_low_pos & ~res_cell_mat_both))];
        list_rest_non_sig_non_mov_val_KO = [list_rest_non_sig_non_mov_val_KO; mean(w_corr_low(adj_sig_corr_low_non_sig & ~res_cell_mat_both))];  
    end
...
%% compare Modulated to non modulated values during running  
...
%% compare Modulated to non modulated values during running  - ALL CORRELATIONS


figure
boxplot([list_run_mov_val_WT',...
    list_run_non_mov_val_WT',...
    list_run_mov_val_KO',...
    list_run_non_mov_val_KO'],...
    [ones(size(list_run_mov_val_WT')),...
    2*ones(size(list_run_non_mov_val_WT')),...
    3*ones(size(list_run_mov_val_KO')),...
    4*ones(size(list_run_non_mov_val_KO'))],'Colors','k')
hold on
s1 = scatter(1,list_run_mov_val_WT',[],'filled','MarkerFaceColor', colors(2,:));
hold on
s2 = scatter(2,list_run_non_mov_val_WT',[],'filled','MarkerFaceColor', 0.5*[1,1,1]);
hold on
s3 = scatter(3,list_run_mov_val_KO',[],'filled','MarkerFaceColor', colors(4,:));
hold on
s4 = scatter(4,list_run_non_mov_val_KO',[],'filled','MarkerFaceColor',  0.5*[1,1,1]);

xlim([0.5,4.5])
xticks([1,2,3,4])
xticklabels({'MovMod','NonMovMod','MovMod','NonMovMod'})
[h_WT,p_WT] = ttest(list_run_mov_val_WT',list_run_non_mov_val_WT');
[h_KO,p_KO] = ttest(list_run_mov_val_KO',list_run_non_mov_val_KO');
[h_Mod,p_Mod] = ttest2(list_run_mov_val_WT',list_run_mov_val_KO');
[h_NonMod,p_NonMod] = ttest2(list_run_non_mov_val_WT',list_run_non_mov_val_KO');
...
ylabel({'Correlation strength'})
title({'Correlation of All', 'Movement modulated or Non modulated cell pairs','During running'})

ttests = [p_WT,p_KO,p_Mod,p_NonMod]';
means = [mean(list_run_mov_val_WT','omitnan'),mean(list_run_mov_val_KO'),mean(list_run_non_mov_val_WT'),mean(list_run_non_mov_val_KO')]';
stds = [std(list_run_mov_val_WT','omitnan'),std(list_run_mov_val_KO'),std(list_run_non_mov_val_WT'),std(list_run_non_mov_val_KO')]';
