...
%                             c_sig = [c_sig;y_AllMat(y_AllMat<y_ThreshMat_neg & (y_close == 0 & y_AllMat <0))];
                              ...
                              corrStatsTable{row,sp+10} = mean(y_AllMat((y_AllMat-y_ThreshMat_neg <-1e-5) & (y_close == 0)));
                        case  2
...
%                             c_sig = [c_sig;y_AllMat(y_AllMat>y_ThreshMat_pos & (y_close == 0) & y_AllMat >0)];
                              ...
                              corrStatsTable{row,sp+10} = mean(y_AllMat(y_AllMat>y_ThreshMat_pos & (y_close == 0) & y_AllMat >0));
                        case  3
...
xline(pdf_list(1).sig_mean,'color',colors(1,:))
xline(pdf_list(2).sig_mean,'color',colors(3,:))
xline(pdf_list(3).sig_mean,'color',colors(2,:))
xline(pdf_list(4).sig_mean,'color',colors(4,:))
...
% set(gca,'Units','inches','InnerPosition',[1 1 2 4.5],'TickDir','out','TickLength',[0.015, 0.025],'Color','none','Box','off','LineWidth',2)
...
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('PDF %s - %s - speedwise - %s.fig',corrNames{corrTypeId},'Ball', corrDir{ccc})))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('PDF %s - %s - speedwise - %s.png',corrNames{corrTypeId},'Ball', corrDir{ccc})))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('PDF %s - %s - speedwise - %s.epsc',corrNames{corrTypeId},'Ball', corrDir{ccc})))
...
% we do 4 comparisons so bonferroni corrected alpha = 0.05/4 = 0.0125

% A WT rest vs running 1 vs 3 
% B KO rest vs running 2 vs 4
% C Rest - WT vs KO   1 vs 2 
% D Running - WT vs KO 3 vs 4
ks(1).comparison = 'A';
ks(2).comparison = 'B';
ks(3).comparison = 'C';
ks(4).comparison = 'D';
[ks(1).h,ks(1).p] = kstest2(pdf_list(1).sig_data,pdf_list(3).sig_data,'alpha',0.0125)
[ks(2).h,ks(2).p] = kstest2(pdf_list(2).sig_data,pdf_list(4).sig_data,'alpha',0.0125)
[ks(3).h,ks(3).p] = kstest2(pdf_list(1).sig_data,pdf_list(2).sig_data,'alpha',0.0125)
[ks(4).h,ks(4).p] = kstest2(pdf_list(3).sig_data,pdf_list(4).sig_data,'alpha',0.0125)


%% test for normality 

[h,p] = kstest(pdf_list(1).sig_data)
[h,p] = kstest(pdf_list(2).sig_data)
[h,p] = kstest(pdf_list(3).sig_data)
[h,p] = kstest(pdf_list(4).sig_data)

% Kruskalwallis test

[p,tbl,stats] = kruskalwallis([pdf_list(1).sig_data;...
                               pdf_list(2).sig_data;...
                               pdf_list(3).sig_data;...
                               pdf_list(4).sig_data],...
                               [1*ones(size(pdf_list(1).sig_data));...
                                2*ones(size(pdf_list(2).sig_data));...
                                3*ones(size(pdf_list(3).sig_data));...
                                4*ones(size(pdf_list(4).sig_data))]);
[c,m] = multcompare(stats,'CType','bonferroni');

% Anova test 
[p_a,tbl_a,stats_a] = anova1([pdf_list(1).sig_data;...
                               pdf_list(2).sig_data;...
                               pdf_list(3).sig_data;...
                               pdf_list(4).sig_data],...
                               [1*ones(size(pdf_list(1).sig_data));...
...
%     saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - Low Speed - %s.fig',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
%     saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - Low Speed - %s.png',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
%     saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - Low Speed - %s.epsc',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
...
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - High Speed - %s.fig',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - High Speed - %s.png',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Fisher test %s - %s - High Speed - %s.epsc',corrNames{corrTypeId},'Ball', corrDir{ccc} )))
...
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Pearson session mean %s - %s - speedwise - %s.fig',corrNames{corrTypeId},'Ball', corrDir{ccc})))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Pearson session mean %s - %s - speedwise - %s.png',corrNames{corrTypeId},'Ball', corrDir{ccc})))
% saveas(gcf,fullfile('U:\eng_research_handata\Athif Mohamed\nexmif_paper\code_final\plots\correlation_analysis_ball',sprintf('Pearson session mean %s - %s - speedwise - %s.epsc',corrNames{corrTypeId},'Ball', corrDir{ccc})))
